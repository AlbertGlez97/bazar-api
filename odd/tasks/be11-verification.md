...
- [ ] **T01 — Preparation.** Fetch, Docker up, `npm install` (first time with `resend`), `prisma generate`.
...
Delegated writer is fixing F1 recursion, F2 build errors, F3 seed guard (P2002 + 42501 with unit tests) and F4 lockfile; then re-run build, lint, unit and e2e and decide the role/superuser question with the user.
