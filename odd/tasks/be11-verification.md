...
### Option A: dedicated runtime role (2026-09-24), one delegated writer, verified by the parent

- Design: two connection levels. The owner role (docker `POSTGRES_USER`) is used only by `prisma migrate`, the provisioning script and the test-DB migration (`DATABASE_URL_MIGRATE` / `DATABASE_URL_TEST_MIGRATE`, falling back to the runtime URLs). A dedicated `NOSUPERUSER NOBYPASSRLS` runtime role (`APP_DB_USER` / `APP_DB_TEST_USER`) is used by the app, `prisma db seed` and every test. `scripts/provision-app-role.mjs` (`db:provision`, `db:provision:test`; `db:migrate:test` also provisions) is idempotent: role, password rotation, grants, default privileges, no access to `_prisma_migrations`. `src/database/rls-role-guard.ts` makes the app refuse to boot on a superuser or BYPASSRLS connection. Setup and SQL checks are in `doc/runtime-database-role.md`.
- Commits (not pushed): `c86c3bc` tenant-extension fixtures, `0d4dff5` `approve` sets `app.context_id`, `59f07e2` runtime role infrastructure.
- Result on the committed HEAD `59f07e2`, verified by the parent in a clean git worktree that excludes the other session's uncommitted work: `tsc --noEmit` exit 0; `npm run lint` exit 0 (2 pre-existing BE-09 warnings); unit 9 files / 72 tests; e2e 18 files / 124 tests, 0 failures, with `test/rls-strict.e2e-spec.ts` passing unchanged. In the working tree that also holds the other session's changes: unit 10 / 76, e2e 18 / 125, all passing.
- Real product bug exposed by enforcing RLS (invisible under the superuser): `BusinessRegistrationService.approve` created the founding Member of a brand-new `contextId` in a transaction that never set `app.context_id`, so Postgres rejected it with SQLSTATE 42501 (2 e2e tests failed with HTTP 500). Fixed in `0d4dff5` by setting the variable (transaction-local) to the new context before the insert; the module comment that described only the application-layer escape hatch was updated. A fixture bug in `test/tenant-extension.e2e-spec.ts` (rows created outside a tenant scope, cleanup on the unscoped client) was fixed in `c86c3bc`.
- Direct SQL on the dev database as the runtime role (parent, outside Prisma): `rolsuper|rolbypassrls|rolcreatedb|rolcreaterole` = `f|f|f|f`; all 11 tenant tables return 0 rows without `app.context_id` while the owner sees 2 Members and 3 Devices; with `bazar-local` set (transaction-local) the role sees 2 Members and 3 Devices, with another context 0; `CREATE TABLE` is denied, `SELECT` on `_prisma_migrations` is denied, `DROP TABLE "Member"` is denied (must be owner).
- Additional wrong blind assumptions: (5) that the public `approve` endpoint could create tenant rows with only an explicit `contextId` in `data` (the database also needs the session variable); (6) that provisioning could revoke `_prisma_migrations` before the first migration (the table does not exist yet, so it is re-run after migrating).
