...
- [x] **T6 — Scope the race probe to approval B and document the username-conflict 502 (non-blocking findings of the native review of T5).** Test and docs only, no production behavior change. One commit `test(business-registration): scope race probe to approval B and document username-conflict 502` (hash: `git log -1 -- odd/tasks/approve-credentials-and-xss-fix.md`).
...
### T6 (single follow-up commit)
- Probe scope. The old probe counted ANY session of the database waiting on a lock in an `Account` INSERT, so an unrelated parallel Account insert could release approval A early. Prisma sends parameterized SQL, so B's username is not in `pg_stat_activity.query` and cannot be matched. Design: while the test runs, `vi.spyOn(prisma, '$transaction')` (same seam as the expiry test) wraps each callback and runs `SELECT set_config('application_name', <tag>, true)` first (transaction-local). Tag A (`race-A-<run uuid>`) is active until A reaches the mocked email, then tag B; each approval gets its own unique tag deterministically. The probe requires exactly one session with B's tag that waits on a Lock in an Account INSERT AND whose `pg_blocking_pids` contains a session with A's tag. Bounded at 10 s; on timeout A is released only to drain, and the test fails with `approval B was never observed blocked by approval A ...; tagged sessions: [...]` (state, wait event, blocked_by, query of both sessions). The spy is restored in `finally`. The test has an explicit 15 s timeout (default 5 s would cut off the diagnostic). All previous assertions are unchanged.
- Evidence, green: `test/business-registration.e2e-spec.ts` 5 consecutive runs, each 1 file / 21 tests passed.
- Evidence, mutation: probe changed to require a session tagged `<tagA>-unrelated` (never exists). The test failed closed after ~13 s with `Error: approval B was never observed blocked by approval A on the Account insert; tagged sessions: [{race-A..., "idle in transaction", blocked_by []}, {race-B..., "active", wait_event_type "Lock", wait_event "transactionid", blocked_by [84700]}]` (20 passed, 1 failed). Mutation reverted (no `unrelated` left in the file); 5 more consecutive green runs (21 passed each).
- Swagger. `OperationExamples.responses` allowed several entries with the same status, but Nest keeps one `ApiResponse` per status, so the generated document kept only the last example (approve: the email-failure 502 and the "Negocio aprobado" 200 were already lost, and adding a third 502 would have hidden the timeout one). `ApiExample` (`src/docs/api-example.decorator.ts`) now groups entries by status into ONE response with a named example each (`examples.outcomeN`, description joined); single-entry statuses keep `example`. `operation-examples.ts` gains the `No se pudo crear el usuario` 502 example (exact title and text from `retryableApproveFailure`) and the timeout description no longer mentions the username case. RED: `test/docs.e2e-spec.ts` new test "documents every approve outcome that shares a status" failed on the old decorator (`TypeError: Cannot convert undefined or null to object`, no `examples`); GREEN with the change (3 passed): 502 examples contain the email-failure, timeout and username-conflict titles, and the 200 examples contain `Negocio aprobado`.
- `doc/api-contract-for-frontend.md` already lists both 502 titles and texts consistently (status table rows for `La aprobación tardó demasiado` and `No se pudo crear el usuario`); not changed, so no JSON/link re-check was needed.
- Runners: `npm.cmd run build` exit 0; `npm.cmd run lint` exit 0 (same 2 pre-existing warnings in `test/sales-conflict.e2e-spec.ts`); `npm.cmd test` 16 files / 159 tests passed; `npm.cmd run test:e2e` 18 files / 141 tests passed (one more than T5: the new docs test).
