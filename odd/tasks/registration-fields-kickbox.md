...
- DTO: `nombre`, `apellidos`, `correo`, `telefono` are trimmed BEFORE validation (`@Transform`), so a blank-only value is a 400. `correo`: `@IsEmail({ ignore_max_length: true })` with the Spanish message `Escribe un correo válido, por ejemplo nombre@dominio.com` plus `@MaxLength(254)` with `El correo no puede tener más de 254 caracteres` (`ignore_max_length` so an over-long address gets only one message). The service stores the correo lower-cased. `telefono`: `@ValidateIf(v !== undefined)`, so `null` is rejected (omit the field), blank is rejected, trimmed value is stored; absent is stored as NULL.
