...
- **Follow-up (later the same day, not covered by this approval):** both findings were addressed in a separate commit (`fix(api): address the two advisory findings of the prefix review`). For R3-bootstrap-capture-ordering, a mutation check first showed that the existing e2e did NOT notice a removed `await` in `src/main.ts` (9/9 still passed with `bootstrap()` un-awaited), so the reviewer's concern about an unguarded ordering was right in substance even though today's code is correct; the e2e now asserts `app.getHttpServer().listening` right after the import and, against the same mutation, fails with `expected false to be true`. For R3-baseurl-case, `baseUrl()` now strips `/api/v1` case-insensitively and drops a query string or fragment, with five new unit cases that failed first (for example `/API/v1` produced `/API/v1/api/v1/business-registration/approve`). That commit has not been through the native review.
