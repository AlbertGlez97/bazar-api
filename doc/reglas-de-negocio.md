...
- BE-11 (multi-tenancy) no se pudo compilar, migrar (`prisma migrate`) ni ejecutar en esta sesión — se escribió y revisó por lectura solamente. Antes de confiar en esta entrega en la otra máquina, correr un smoke test explícito de: (a) que la migración de RLS aplique sin error, (b) que un `findMany` normal de un request autenticado real solo muestre filas de su propio contextId, (c) que `prisma/seed.ts` siga insertando correctamente bajo las nuevas políticas FORCE RLS. La política estricta de RLS (deniega por defecto) y el filtro de contextId en el `$queryRaw` de `deudas.service.ts` ya quedaron resueltos en código en esta sesión de seguimiento — no siguen pendientes de smoke-test, solo de la verificación general de arriba.
